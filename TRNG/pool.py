...
class EntropyPool:
    ...
    def get_hashed_bytes(self):
        # Convert bits to byte string for hashing
    ...
    def get_final_sequence(self, method='hash'):
        if method == 'von_neumann':
            return self._von_neumann_extractor(self.raw_bits)
        elif method == 'hash':
            # Returns a binary string representation of the hash
